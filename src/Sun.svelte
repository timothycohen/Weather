<script>
  ...
  $: {
    if (sunriseMins && foreignMins && sunsetMins && foreignMins > sunriseMins && foreignMins < sunsetMins) {
  ...
  $: {
    if (sunriseMins && foreignMins) {
      if (sunriseMins < foreignMins) {
        timeToSunrise = minsTo24(sunriseMins + 1440 - foreignMins);
  ...
  $: {
    if (sunsetMins && foreignMins) {
      if (sunsetMins < foreignMins) {
        timeToSunset = minsTo24(sunsetMins + 1440 - foreignMins);
...
<div class="canvasContainer">
  <canvas width="500" height="100" bind:this={canvas} />
</div>
